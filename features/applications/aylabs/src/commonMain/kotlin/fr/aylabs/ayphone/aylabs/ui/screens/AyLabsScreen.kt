...
fun AyLabsScreen(
    ...
    AyAppScaffold(
        title = "AyLabs",
        containerColor = AyApp.AYLABS.color,
        onBackClick = onBackClick,
    ) { padding ->
        Column(modifier = Modifier.padding(padding)) {
            when (val state = uiState) {
                is AyLabsState.Initial,
                is AyLabsState.Loading -> {
                    Column(
                ...
                is AyLabsState.Success -> {
                    AyLabsReadyScreen(info = state.info)
                }
            }
        }
    }
